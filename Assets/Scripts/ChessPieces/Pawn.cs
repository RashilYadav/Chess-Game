...
public class Pawn : ChessPiece
{
    public override List<Vector2Int> GetAvailableMoves(ref ChessPiece[,] board, int tileCountX, int tileCountY)
    {
        ...
        int direction = (team == 0) ? 1 : -1; //Direction will be up if the team is white or will be down if the team is black
        ...
        if(moveList.Count > 0)
        {
            ...
            if (board[lastMove[1].x, lastMove[1].y].type == ChessPieceType.Pawn) // if the last piece moved was a pawn
            {
                if(Mathf.Abs(lastMove[0].y - lastMove[1].y) == 2) // if the last move was a +2 in either direction
                {
                    ...
                    {
                        if(lastMove[1].y == currentY) // if both pawns are on the same Y,  if the pawn is either on the left or on right of the current pawn 
                        {
